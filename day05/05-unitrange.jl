...
# unused
function intersect(sr::UnitRange, mp::Mapping)::UnitRange
...
function applymappings!(seedmaps::Array{Tuple{UnitRange, UnitRange}}, mappings::Array{Mapping})
    rmaps = Tuple{UnitRange, UnitRange}[]
    extramaps = Tuple{UnitRange, UnitRange}[]
    ...
    while seedmaps |> !isempty
        ...
        for m in mappings
            ...
            if first(m) <= first(t) <= last(m) ||
                ...
                if tail_left > 0
                    sr_left = range(first(t), length=tail_left)
                    #debugmsg("Left tail: $(sr_left)")
                    push!(seedmaps, (s, sr_left))
                end
                if tail_right > 0
                    sr_right = range(last(m)+1, length=tail_right)
                ...
                push!(rmaps, (s, range(newrange_start, length=newrange_length)))
...
function seedparsev1(sline)
    seedmap = Tuple{UnitRange, UnitRange}[]
